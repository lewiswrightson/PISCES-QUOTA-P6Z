MODULE p4zsms
   ...
   !! $Id: p4zsms.F90 13233 2020-07-02 18:34:16Z aumont $ 
...
908   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampismass in configuration namelist' )
   ...
   SUBROUTINE p4z_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN
        ...
        ENDIF
         ...
         ! PISCES-QUOTA Explict Diazotroph PFT
         IF( ln_p6z ) THEN
            ! Read the size of the different phytoplankton groups
            ! If not in the restart file, they are set to 1
            IF( iom_varid( numrtr, 'sizedz', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_autoglo, 'sizedz' , sizedz(:,:,:)  )
            ELSE
               sizedz(:,:,:) = 1.
            ENDIF
            IF( iom_varid( numrtr, 'vnfmax_dz', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_autoglo, 'vnfmax_dz' , vnfmax_dz(:,:,:)  )
            ELSE
               vnfmax_dz(:,:,:) = 1.
            ENDIF
        ENDIF
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN
         ...
         IF( ln_p6z ) THEN
            CALL iom_rstput( kt, nitrst, numrtw, 'sizep', sizep(:,:,:) )  ! Size of picophytoplankton
            CALL iom_rstput( kt, nitrst, numrtw, 'sizedz', sizedz(:,:,:) )
         ENDIF
      ENDIF
      !
   ...
   SUBROUTINE p4z_dmp( kt )
      ...
      REAL(wp) ::  po4mean = 2.165     ! mean value of phosphate
   ...
   SUBROUTINE p4z_chk_mass( kt )
      ...
      ! --------------------------------------------------------------------------------
   IF( ln_p4z .OR. ln_p5z) THEN  ! Implicit Nitrogen fixation
