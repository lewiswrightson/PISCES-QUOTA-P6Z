MODULE sms_pisces   
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   tgfunc3   !: Temp. dependancy of picophytoplankton rates
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   tgfuncnfix !: Temp dependency Diazotrophs

   !!----------------------------------------------------------------------
   !! NEMO/TOP 4.0 , NEMO Consortium (2018)
   !! $Id: sms_pisces.F90 12349 2020-01-28 16:17:13Z aumont $ 
...
CONTAINS
   ...
   INTEGER FUNCTION sms_pisces_alloc()
      ...
      INTEGER ::   ierr(13)        ! Local variables
      !!----------------------------------------------------------------------
      ierr(:) = 0
      ...
      IF( ln_p4z .OR. ln_p5z .OR. ln_p6z ) THEN
         ...
         !
         !* Temperature dependancy of SMS terms
         ALLOCATE( tgfunc(jpi,jpj,jpk)  , tgfunc2(jpi,jpj,jpk),     &
            &      tgfunc3(jpi,jpj,jpk) , tgfuncnfix(jpi,jpj,jpk),  STAT=ierr(6) )
         !
         !* Sinkong speed
      ...
      IF( ln_p5z .OR. ln_p6z ) THEN
         ...
         ALLOCATE( sizen(jpi,jpj,jpk), sizep(jpi,jpj,jpk),         &
           &       sized(jpi,jpj,jpk), sizena(jpi,jpj,jpk),        &
           &       sizepa(jpi,jpj,jpk), sizeda(jpi,jpj,jpk),       STAT=ierr(10) )
      ENDIF
      ! PISCES QUOTA Explicit Diazotroph
      IF( ln_p6z ) THEN
         !
         ALLOCATE(ediaz(jpi,jpj,jpk)   , ediazm(jpi,jpj,jpk),       STAT=ierr(11) )
         !* size of diazotroph cells
         ALLOCATE(  sizedz(jpi,jpj,jpk), sizedza(jpi,jpj,jpk),      STAT=ierr(12) )
         !* Maximum nutrient uptake by diazotroph
         ALLOCATE( vnfmax_dz(jpi,jpj,jpk),  STAT=ierr(13) )
      ENDIF
      !
