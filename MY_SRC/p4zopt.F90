MODULE p4zopt
   ...
   !! $Id: p4zopt.F90 13233 2020-07-02 18:34:16Z aumont $ 
...
CONTAINS
   ...
   SUBROUTINE p4z_opt( kt, knt )
      ...
      REAL(wp), ALLOCATABLE, DIMENSION(:,:  ) :: zetmp5, zetmp6
      ...
      IF( ln_p5z .OR. ln_p6z  )   ALLOCATE( zetmp5(jpi,jpj) )
      IF( ln_p6z ) ALLOCATE( zetmp6(jpi,jpj) )
      ...
      IF( ln_p5z .OR. ln_p6z )   zchl3d(:,:,:) = zchl3d(:,:,:)    + trb(:,:,:,jppch)
      !
      ...
      IF( l_trcdm2dc ) THEN                     !  diurnal cycle
         ...
         IF( ln_p6z ) THEN  ! Explicit Diazotrophy
            DO jk = 1, nksrp
              ediaz  (:,:,jk) =  1.85 * ze1(:,:,jk) + 0.69 * ze2(:,:,jk) + 0.46 * ze3(:,:,jk)
            END DO
         ENDIF
         ! SW over the ice free zone of the grid cell. This assumes that
         ! SW is zero below sea ice which is a very crude assumption that is 
      ...
      ELSE ! no diurnal cycle
         ...
         IF( ln_p6z ) THEN
            DO jk = 1, nksrp
              ediaz(:,:,jk) =  1.85 * ze1(:,:,jk) + 0.69 * ze2(:,:,jk) + 0.46 * ze3(:,:,jk) ! Explicit Diazotrophs (PISCES QUOTA)
            END DO
         ENDIF
         etot_ndcy(:,:,:) =  etot(:,:,:) 
      ENDIF
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF( gdepw_n(ji,jj,jk+1) <= hmld(ji,jj) ) THEN
                  zetmp1 (ji,jj) = zetmp1 (ji,jj) + etot     (ji,jj,jk) * e3t_n(ji,jj,jk) ! Actual PAR
                  zetmp2 (ji,jj) = zetmp2 (ji,jj) + etot_ndcy(ji,jj,jk) * e3t_n(ji,jj,jk) ! Par averaged over 24h
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF( gdepw_n(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
                  zetmp3 (ji,jj) = zetmp3 (ji,jj) + enano    (ji,jj,jk) * e3t_n(ji,jj,jk) ! Nanophytoplankton
                  zetmp4 (ji,jj) = zetmp4 (ji,jj) + ediat    (ji,jj,jk) * e3t_n(ji,jj,jk) ! Diatoms 
                  zdepmoy(ji,jj) = zdepmoy(ji,jj) +                       e3t_n(ji,jj,jk)
               ENDIF
            END DO
         END DO
      ...
      IF( ln_p5z .OR. ln_p6z ) THEN
         ...
         DO jk = 1, nksrp
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gdepw_n(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
                     zetmp5(ji,jj)  = zetmp5 (ji,jj) + epico(ji,jj,jk) * e3t_n(ji,jj,jk)
                  ENDIF
               END DO
            END DO
         END DO
...
!
      IF( ln_p6z ) THEN
         ! Explicit diazotrophs when using PISCES-QUOTA (P6Z)
         zetmp6 (:,:) = 0.e0
         DO jk = 1, nksrp
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gdepw_n(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
                     zetmp6(ji,jj)  = zetmp6 (ji,jj) + ediaz(ji,jj,jk) * e3t_n(ji,jj,jk)
                  ENDIF
               END DO
            END DO
         END DO
         ...
         DO jk = 1, nksrp
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gdepw_n(ji,jj,jk+1) <= hmld(ji,jj) ) THEN
                     z1_dep = 1. / ( zdepmoy(ji,jj) + rtrn )
                     ediazm(ji,jj,jk) = zetmp6(ji,jj) * z1_dep
                  ENDIF
               END DO
            END DO
         END DO
      ...
      IF( ln_p6z    )   DEALLOCATE( zetmp5, zetmp6 )
